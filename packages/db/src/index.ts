...
export async function updateQuoteStatus(id:string,status:QuoteStatus){const sql=client();if(!sql)return false;try{const rows=await sql`update quotes set status=${status},updated_at=now() where id=${id}`;return rows.count>0;}finally{await sql.end();}}
export async function updatePublicQuoteStatus(id:string,token:string,status:"approved"|"rejected"){const sql=client();if(!sql)return false;try{const rows=await sql`update quotes set status=${status},updated_at=now() where id=${id} and public_token=${token} and status in ("draft","sent")`;return rows.count>0;}finally{await sql.end();}}
